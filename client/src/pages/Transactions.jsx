...
export default function Transactions() {
  ...
  useEffect(() => { const beneficiary = new URLSearchParams(location.search).get("beneficiary"); if (beneficiary) { setForm((current) => ({ ...current, toAccount: beneficiary })); setMode("transfer"); } }, [location.search]);
  ...
  async function reverse(event) {
    event.preventDefault();
    if (busy || !reversalTarget || !reversalReason.trim()) return;
    if (!window.confirm(`Reverse ${reversalTarget.REFERENCE_NO}? This creates a compensating ledger entry.`)) return;
    setBusy(true);
    try { const result = await api.post(`/transactions/${reversalTarget.TRANSACTION_ID}/reversal`, { reason: reversalReason.trim() }); setReceipt({ reference: result.data.reversalReference || "Reversal submitted", type: "REVERSAL", amount: reversalTarget.AMOUNT, status: "SUCCESS" }); setReversalTarget(null); setReversalReason(""); await load(); }
    catch (requestError) { setError(messageFrom(requestError)); }
    finally { setBusy(false); }
  }
  ...
  return <>
    ...
    {reversalTarget && <form className="card mb-5 border-amber-200" onSubmit={reverse}><h2 className="mb-3 font-bold">Reverse transaction {reversalTarget.REFERENCE_NO}</h2><p className="mb-3 text-sm text-slate-600">{reversalTarget.TRANSACTION_TYPE} · {money(reversalTarget.AMOUNT)} · original ledger row is preserved.</p><label className="field">Required reason<textarea required minLength="5" value={reversalReason} onChange={(event) => setReversalReason(event.target.value)} /></label><div className="mt-3 flex gap-2"><button className="btn-primary" disabled={busy}>{busy ? "Reversing…" : "Confirm reversal"}</button><button type="button" className="btn-secondary" onClick={() => setReversalTarget(null)}>Cancel</button></div></form>}
    ...
    {!data?.items.length ? <Empty /> : <div className="table-wrap"><table className="data-table"><thead><tr><th>Reference</th><th>Type</th><th>Account</th><th>Amount</th><th>Balance after</th><th>Time</th><th>Status</th>{["ADMIN", "MANAGER"].includes(user.role) && <th>Action</th>}</tr></thead><tbody>{data.items.map((transaction) => <tr key={transaction.TRANSACTION_ID}><td className="font-mono text-xs">{transaction.REFERENCE_NO}</td><td>{transaction.TRANSACTION_TYPE.replaceAll("_", " ")}</td><td>••••{transaction.ACCOUNT_NUMBER.slice(-4)}</td><td>{money(transaction.AMOUNT)}</td><td>{money(transaction.NEW_BALANCE)}</td><td>{new Date(transaction.TRANSACTION_DATE).toLocaleString()}</td><td><Status value={transaction.STATUS} />{transaction.REVERSAL_ID && <span className="ml-1 text-xs text-amber-700">Reversed</span>}</td>{["ADMIN", "MANAGER"].includes(user.role) && <td>{["DEPOSIT", "WITHDRAWAL"].includes(transaction.TRANSACTION_TYPE) && transaction.STATUS === "SUCCESS" && !transaction.REVERSAL_ID && <button className="text-amber-700" disabled={busy} onClick={() => setReversalTarget(transaction)}>Reverse</button>}</td>}</tr>)}</tbody></table></div>}
