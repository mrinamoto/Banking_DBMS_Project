...
INSERT INTO loan_types(type_name,short_description,detailed_description,minimum_annual_income,processing_fee_percentage,eligibility_summary,required_document_summary,interest_method,min_amount,max_amount,annual_interest_rate,min_term_months,max_term_months) VALUES('Personal Loan','Flexible personal finance','For education, medical and household needs with reducing-balance interest.',300000,1.00,'Age 21+ with stable income and satisfactory KYC','National ID, income proof, bank statement','REDUCING_BALANCE',50000,1000000,11.50,6,60);
INSERT INTO loan_types(type_name,short_description,detailed_description,minimum_annual_income,processing_fee_percentage,eligibility_summary,required_document_summary,interest_method,min_amount,max_amount,annual_interest_rate,min_term_months,max_term_months) VALUES('Home Loan','Home purchase finance','Long-term home finance for eligible customers; approval remains subject to review.',600000,0.50,'Stable income, verified address and clean KYC','National ID, income proof, property documents','REDUCING_BALANCE',500000,10000000,8.75,12,240);
...
INSERT INTO employees(branch_id,employee_code,first_name,last_name,national_id,job_title,email,phone,salary) SELECT branch_id,'M-ID-001','Ann','Noor','EMP-NID-001','Branch Manager','ann.noor@example.test','01810000001',85000 FROM branches WHERE branch_code='DHK-001';
INSERT INTO employees(branch_id,employee_code,first_name,last_name,national_id,job_title,email,phone,salary) SELECT branch_id,'E-ID-001','Rafi','Ahmed','EMP-NID-002','Bank Officer','rafi.ahmed@example.test','01810000002',52000 FROM branches WHERE branch_code='DHK-001';
