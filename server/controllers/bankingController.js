...
const {assertAccountAccess,assertBranchId,assertLoanAccess,setClientIdentifier}=require('../utils/authorization');
...
async function listBranches(req,res,next){try{await withConnection(getConnection,async c=>{const r=await c.execute(`SELECT branch_id,branch_code,branch_name,city,address,phone,status FROM branches WHERE LOWER(branch_name||' '||branch_code||' '||city) LIKE :search ORDER BY branch_name`,{search:`%${String(req.query.search||'').toLowerCase()}%`});res.json(r.rows)});}catch(e){next(e)}}
...
async function lookups(req,res,next){try{await withConnection(getConnection,async c=>{const branchId=['MANAGER','EMPLOYEE'].includes(req.user.role)?req.user.branchId:null;const [branches,types,loanTypes]=await Promise.all([c.execute(`SELECT branch_id id,branch_name label FROM branches WHERE status='ACTIVE' AND (:branchId IS NULL OR branch_id=:branchId) ORDER BY branch_name`,{branchId}),c.execute(`SELECT account_type_id id,type_name label,min_balance FROM account_types WHERE status='ACTIVE' ORDER BY type_name`),c.execute(`SELECT loan_type_id id,type_name label,min_amount,max_amount,annual_interest_rate,min_term_months,max_term_months FROM loan_types WHERE status='ACTIVE' ORDER BY type_name`)]);res.json({branches:branches.rows,accountTypes:types.rows,loanTypes:loanTypes.rows})});}catch(e){next(e)}}
...
module.exports={listBranches,createBranch,listAccounts,openAccount,changeAccountStatus,transactionOperation,transfer,listTransactions,listLoans,applyLoan,decideLoan,payLoan,loanPayments,lookups,listEmployees,createEmployee,updateEmployee,setEmployeeStatus,reports,audit};
