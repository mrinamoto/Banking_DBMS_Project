...
async function getDashboardStats(req,res,next){try{await withConnection(getConnection,async c=>{
  const binds={customerId:req.user.role==='CUSTOMER'?req.user.customerId:null,branchId:req.user.role==='MANAGER'?req.user.branchId:null};
  const [customers,accounts,employees,loans,transactions,recent]=await Promise.all([
    ...
    c.execute(`SELECT NVL(SUM(t.amount),0) volume FROM transactions t JOIN accounts a ON a.account_id=t.account_id WHERE (:customerId IS NULL OR a.customer_id=:customerId) AND (:branchId IS NULL OR a.branch_id=:branchId)`,binds),
